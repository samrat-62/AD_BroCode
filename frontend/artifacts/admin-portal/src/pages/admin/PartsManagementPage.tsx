...
import { useListParts, useListPartCategories, useCreatePart, useUpdatePart, useDeletePart, useCreatePartCategory, useGetLowStockParts } from "@workspace/api-client-react";
              ...
              {isLoading ? <div className="p-6 space-y-3">{[1,2,3,4,5].map(i => <Skeleton key={i} className="h-12 w-full" />)}</div> : (
                <Table>
                  ...
                  <TableBody>
                    ...
                    {(data?.data ?? []).length === 0 && <TableRow><TableCell colSpan={7} className="text-center py-10 text-muted-foreground">No parts found.</TableCell></TableRow>}
                  </TableBody>
                </Table>
              )}
